...
9. Als letzten Schritt der Installation geben wir nun die oben festgelegten Zugangsdaten für die MySQL-Datenbank (siehe Schritt "Vorbereiten der Datenbank") ein. Die Werte lauten: Server: ``127.0.0.1`` (bei Problemen eventuell auf ``localhost`` umändern). Datebank-Benutzer: ``prestashop``Database name: ``prestashop`` und Password: ``meinpasswort``. Den Wert Table-Prefix können wir ignorieren. Klicken Sie dann auf 'Test your database connection now!', um die Datenbankverbindung zu überprüfen. {% include imgclick.html alt="41 Prestashop Installer Mysql" src="/img/2018-06-18-prestashop-installieren/41-prestashop-installer-mysql.png" %}
